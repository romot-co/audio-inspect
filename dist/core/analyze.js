 ...
 * 音声データから特徴量を抽出する
 */
export async function analyze(audio, feature) {
    try {
        // 入力検証
        ...
        throw new AudioInspectError('PROCESSING_ERROR', `特徴量の抽出に失敗しました: ${message}`, error);
    }
}
/**
 * AudioDataの入力検証
...
function validateAudioData(audio) {
    ...
    if (typeof audio.numberOfChannels !== 'number' ||
