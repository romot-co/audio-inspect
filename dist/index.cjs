...
__export(index_exports, {
  ...
  createAudioInspectNodeWithDefaults: () => createAudioInspectNodeWithDefaults,
  decibelsToAmplitude: () => decibelsToAmplitude,
  getChannelData: () => getChannelData,
  getCrestFactor: () => getCrestFactor,
  getDefaultProcessorUrl: () => getDefaultProcessorUrl,
  getEnergy: () => getEnergy,
  getFFT: () => getFFT,
  getLUFS: () => getLUFS,
  ...
  isAudioInspectError: () => isAudioInspectError,
  isPowerOfTwo: () => isPowerOfTwo,
  load: () => load,
  nextPowerOfTwo: () => nextPowerOfTwo,
  stream: () => stream,
  streamWithFallback: () => streamWithFallback
});
...
function normalize(audioData) {
  ...
  const normalizedChannels = audioData.channelData.map((channelData) => {
    ...
    for (let i = 0; i < channelData.length; i++) {
      const sample = channelData[i];
      normalized[i] = sample !== void 0 ? sample / maxAmplitude : 0;
    }
    return normalized;
  });
  return {
...
function decibelsToAmplitude(db, reference = 1) {
...
function calculateFrameZCRs(channelData, frameSizeSamples, hopSizeSamples, normalize2 = true) {
  const dataLength = channelData.length;
  if (dataLength < frameSizeSamples) {
  ...
  for (let i = 0; i < frameCount; i++) {
    ...
    zcrs[i] = normalize2 ? crossings / Math.max(1, end - start - 1) : crossings;
  }
  return zcrs;
}
