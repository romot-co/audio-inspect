...
async function calculateCoherence(
  ...
  for (let i = 0; i < coherence.length; i++) {
    ...
    coherence[i] = denominator > 1e-10 
      ? (crossMag * crossMag) / denominator 
      : 0;
  }
...
export async function getStereoAnalysis(
  ...
  const correlation = (stdL > 1e-10 && stdR > 1e-10) 
    ? covariance / (stdL * stdR) 
    : 0;
  ...
  // メトリクスの計算
  const width = (energyMid + energySide) > 1e-10 
    ? energySide / (energyMid + energySide) 
    : 0;
    
  const balance = (energyL + energyR) > 1e-10 
    ? (energyR - energyL) / (energyL + energyR) 
    : 0;
    
  const midSideRatio = energySide > 1e-10 
    ? 10 * Math.log10(energyMid / energySide) 
    : Infinity;
  ...
  if (calculatePhase && frameSize < audio.length) {
    ...
    for (let i = 1; i < leftFFT.phase.length; i++) { // DC成分を除外
      const leftMag = leftFFT.magnitude[i] || 0;
      const rightMag = rightFFT.magnitude[i] || 0;
      const leftPhase = leftFFT.phase[i] || 0;
      const rightPhase = rightFFT.phase[i] || 0;
    ...
    result.phaseDifference = weightSum > 1e-10 
      ? (phaseDiffSum / weightSum) * 180 / Math.PI 
      : 0;
  }
  ...
  if (calculateILD) {
    ...
    result.ild = (rmsL > 1e-10 && rmsR > 1e-10) 
      ? 20 * Math.log10(rmsR / rmsL) 
      : 0;
  }
