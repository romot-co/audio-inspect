...
// 複合信号を生成
function createComplexSignal(sampleRate = 44100, duration = 1) {
...
describe('Integration Tests', () => {
    describe('Full workflow with complex signal', () => {
        it('should analyze complex audio signal successfully', async () => {
            ...
            // ピーク検出
            const peaks = await analyze(audio, (audio) => getPeaks(audio, {
                count: 10,
                threshold: 0.8,
                minDistance: 1000
            }));
    ...
    describe('Multi-channel audio analysis', () => {
        it('should analyze stereo audio', async () => {
            ...
            // 各チャンネルを個別に解析
            const leftPeaks = await analyze(stereoAudio, (audio) => getPeaks(audio, { channel: 0, threshold: 0.5 }));
            const rightPeaks = await analyze(stereoAudio, (audio) => getPeaks(audio, { channel: 1, threshold: 0.5 }));
        ...
        it('should analyze averaged channels', async () => {
            ...
            // 全チャンネル平均で解析
            const averagedPeaks = await analyze(audio, (audio) => getPeaks(audio, { channel: -1, threshold: 0.3 }));
    ...
    describe('Performance and edge cases', () => {
        ...
        it('should handle different sample rates', async () => {
            ...
            for (const sampleRate of sampleRates) {
                ...
                const peaks = await analyze(audio, (audio) => getPeaks(audio, { count: 5, threshold: 0.5 }));
                const rms = await analyze(audio, getRMS);
                expect(peaks.peaks.length).toBeGreaterThan(0);
                expect(rms).toBeGreaterThan(0);
